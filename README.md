...
| 结构 | 类图（分析/设计/界面类图）、对象图、包图、构件图、部署图、架构图（体系结构四视图） |
| 测试/管理 | 控制流图（McCabe 流图）、甘特图、PERT 网络图 |

## 特性

- **先建模后翻译**：每种图附课件方法要点（BCE 三步、分析类图五步、活动图决策边互斥等），再产出 PlantUML 语法
- **默认课件风格**：白底标题、矩形构造型参与者（boundary/control 名称下加横线）、顺序消息顺号编号＋返回虚线不编号、黄色批注、去模板印记
- **架构图支持**：按 SE8 体系结构四视图（逻辑/运行/开发/物理），包图+构件图+部署图组合产出，供给接口左、需求接口右
